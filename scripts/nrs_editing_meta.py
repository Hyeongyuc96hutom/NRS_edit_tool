...
def main():
    ...
    with open('./editing_log_meta.json') as json_file:
        log_data = json.load(json_file)

    total_meta_df = pd.DataFrame([], columns=['video_name', 'frameRate', 'width', 'height', 'totalFrame', 'frame_cnt'])
    ...
    for (root, dirs, files) in os.walk(input_path): # 비디오
        ...
        for file in files:
            ...
            ## get frame cnt form logging file
            frame_cnt = log_data[os.path.basename(target_video)]['frame_cnt']
            ...
            total_meta_df = total_meta_df.append(meta_df, ignore_index=True)
            
            print(total_meta_df)

            total_meta_df.to_csv('./meta_info.csv')
