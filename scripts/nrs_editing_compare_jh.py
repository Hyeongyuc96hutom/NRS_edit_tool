...
# 22.03.21 jh 추가 - inference 1 fps 단위 진행.
...
def extract_video_marking_clip(video_path, results_dir, start_time, end_time, mark):
    from core.utils.ffmpegHelper import ffmpegHelper

    ffmpeg_helper = ffmpegHelper(video_path, results_dir)
    ffmpeg_helper.extract_video_marking_clip(start_time, end_time, mark)
...
def extract_video_from_json(video_path, json_path, editted_video_dir, video_fps, padding_sec=3):
    ...
    for i, (start_idx, end_idx, code) in enumerate(target_clipping_time, 1):
        ...
        temp_clip_dir = os.path.join(process_dir, 'clips', '{}-{}'.format(start_idx, end_idx))
        editted_video_path = os.path.join(process_dir, '{}-{}.mp4'.format(start_idx, end_idx))
        
        os.makedirs(process_dir, exist_ok=True)
        os.makedirs(temp_clip_dir, exist_ok=True)
        
        pre_start_idx, pre_end_idx = start_idx - padding_frame, start_idx - 1
        post_start_idx, post_end_idx = end_idx + 1 , end_idx + padding_frame

        pre_start_idx = 0 if pre_start_idx < 0 else pre_start_idx
        pre_end_idx = 0 if pre_end_idx < 0 else pre_end_idx

        start_time, end_time = idx_to_time(start_idx, video_fps), idx_to_time(end_idx, video_fps) # conver to time
        pre_start_time, pre_end_time = idx_to_time(pre_start_idx, video_fps), idx_to_time(pre_end_idx, video_fps) # conver to time
        post_start_time, post_end_time = idx_to_time(post_start_idx, video_fps), idx_to_time(post_end_idx, video_fps) # conver to time
        
        print('\n\n[{}] \t {} - {} - {}'.format(i, start_time, end_time, code))
        extract_video_marking_clip(video_path, temp_clip_dir, pre_start_time, pre_end_time, mark='-')
        extract_video_marking_clip(video_path, temp_clip_dir, start_time, end_time, mark=code)
        extract_video_marking_clip(video_path, temp_clip_dir, post_start_time, post_end_time, mark='-')
...
def extract_video_from_json_using_frames():
    from subprocess import Popen, PIPE
    
    # https://syki66.github.io/blog/2021/04/18/picture-to-video.html

    cmd_list = ['ffmpeg',  '-r', '30', '-i', '/NRS_editing/frames/frame-0000000000.jpg', '-vcodec', 'libx264', './output.mp4']
    cmd = ' '.join(cmd_list)
...
def main():
    ...
    input_path = '/nas1/ViHUB-pro/input_video/ViHUB_0321'
    ...
    for (root, dirs, files) in os.walk(input_path):
        ...
        for file in files:
            ...
            # predict_csv_path = '/nas1/ViHUB-pro/results/inference_json/ViHUB_0315/severance_1st/211025_Vihub-B4/L_28/01_VIHUB1.2_B4_L_28_01/01_VIHUB1.2_B4_L_28_01.csv'
            ...
            # # extract_video_from_json(video_path=target_video, json_path=compare_save_path, editted_video_dir=os.path.join(edited_video_output_path, 'marking'), video_fps=frameRate, padding_sec=3)
            # extract_video_from_json_using_frames()
