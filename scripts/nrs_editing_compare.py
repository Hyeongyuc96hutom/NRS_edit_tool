...
def return_gt(video_name):
    ...
    gt_paths = glob.glob(os.path.join('/nas1/OOB_Project/annotation/Gastrectomy/Lapa/v3', '*', 'NRS', '*.json'))
...
def main():
    ...
    inference_csv_path = '/NRS_editing/NRS_editing_hg/mobilenet-set1-apply1/TB_log/version_4/inference_results'
    inference_csv = glob.glob(os.path.join(inference_csv_path, '*', '*', '*.csv')) # 66 case. 152ea

    '''
    # check video name
    print(len(inference_csv))

    s = []
    
    for p in inference_csv:
        s.append(os.path.splitext(os.path.basename(p))[0])
    
    s = natsort.natsorted(s)
    for ss in s :
        print()
    '''

    # input_path = '/data3/Public/ViHUB-pro/input_video'
    input_path = '/nas1/ViHUB-pro/input_video/ViHUB_66_case'
    ...
    qa_helper = QAHelper(inference_interval)

    # logging
    cnt = 0
    total_log_df = pd.DataFrame([])
    ...
    for (root, dirs, files) in os.walk(input_path):
        ...
        for file in files:
            ...
            '''
            if not core_output_path.split('/')[-1] == '01_GS3_06_R_165_01':
                continue
            ...
            ## inference (비디오 단위) -> 저장 디렉토리 & result csv 생성 x`
            ...
            os.makedirs('/nas1/ViHUB-pro/results/inference_json/ViHUB_66_case_QA_clip', exist_ok=True)
            
            log_df = qa_helper.excute(target_video, gt_path, inference_json_path=report_annotation_save_path, output_dir=inference_json_output_path)

            # logging
            total_log_df = total_log_df.append(log_df, ignore_index=True)
            total_log_df.to_csv(os.path.join(base_output_path, 'inference_json', 'ViHUB_66_case.csv'))
